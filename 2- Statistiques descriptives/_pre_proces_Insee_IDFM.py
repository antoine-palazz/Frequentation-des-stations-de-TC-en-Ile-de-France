...
def data_INSEE_stations():
    with io.capture_output() as captured:
        ...
        elements_ajout = elements_uniques.drop(
            columns=[
                "Geo Shape_y",
                "gares_id_y",
                "nom_long_y",
        ...
        nouveaux_noms = {
            col: col.rstrip("_x") if col.endswith("_x") else col
            for col in elements_ajout.columns
        }
        ...
        stations_socio_RER = gpd.GeoDataFrame(
            pd.concat([stations_socio_RER, elements_ajout], ignore_index=True)
        )
        # On retire les stations en trop pour la suite de l'analyse
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Creil"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Chantilly-Gouvieux"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Orry-la-Ville-Coye-la-Forêt"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "La Borne Blanche"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Malesherbes"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Igny"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Bièvres"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Vauboyen"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Jouy-en-Josas"
        ]
        stations_socio_RER = stations_socio_RER[
            stations_socio_RER["nom_long"] != "Petit-Jouy-les-Loges"
        ]
