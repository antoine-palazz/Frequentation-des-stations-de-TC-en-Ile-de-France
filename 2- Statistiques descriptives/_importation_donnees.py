...
def donnees_archives():
    ...
    data_archives.rename(
        columns=dict(zip(data_archives.columns, ["date", "lien"])), inplace=True
    )
...
def donnees_stations_toutes():
    ...
    stations = gpd.GeoDataFrame(
        stations, geometry=[Point(xy) for xy in zip(stations.x, stations.y)]
    )
...
def scrapping():
    ...
    for row in soup.find_all("tr"):
        ...
        if len(cells) > 1:
            ...
            if station_link:
                ...
                lignes.append(
                    [
                        clean_text(link["title"].replace("Ligne", ""))
                        for cell in cells[1:]
                        for link in cell.find_all("a", title=True)
                        if "Ligne" in link["title"]
                    ]
                )
    ...
    for url in url_gares:
        ...
        bus_clean = [
            num
            for num in numeros_lignes_bus
            if (any(i.isdigit() for i in num) or len(num) < 3)
        ]
